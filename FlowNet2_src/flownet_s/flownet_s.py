...
# (Yuliang) Use TensorFlow built-in downsample instead
...
# (Yuliang) Replace antipad() with crop_features() to satisfy arbitrary sizes
class FlowNetS(Net):
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                ...
                with slim.arg_scope([slim.conv2d], weights_regularizer=weights_regularizer):
                    ...
                    with slim.arg_scope([slim.conv2d_transpose], biases_initializer=None):
                        ...
                        deconv2 = crop_features(deconv2, tf.shape(conv_2))
                        upsample_flow3to2 = crop_features(upsample_flow3to2, tf.shape(conv_2))
    ...
    def loss(self, flow, predictions):
        ...
        downsampled_flow6 = tf.image.resize_bilinear(flow, size)
        ...
        downsampled_flow5 = tf.image.resize_bilinear(flow, size)
        ...
        downsampled_flow4 = tf.image.resize_bilinear(flow, size)
        ...
        downsampled_flow3 = tf.image.resize_bilinear(flow, size)
        ...
        downsample_flow2 = tf.image.resize_bilinear(flow, size)
